...
export function tickHacknet(ns) {
  ...
  if (nodeCount < maxNodes && money() >= newNodeCost) {
    ...
    logPropulsion(ns, "HACKNET", "BUY", `node-${index}  cost: $${ns.format.number(newNodeCost)}`);
  }
  ...
  while (upgraded) {
    ...
    for (const opt of options) {
      ...
      if (success) {
        logPropulsion(ns, "HACKNET", "UP", `node-${opt.i}  ${opt.type.padEnd(6)}  $${ns.format.number(opt.cost)}`);
        upgraded = true;
        break;
      }
    }
