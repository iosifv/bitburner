...
export function setTask(ns, info, task) {
  ...
  const combat = info.str + info.def + info.dex + info.agi;
  ns.print(`ASSIGN   ${info.name.padEnd(15)} ${`[${combat}]`.padEnd(8)} → ${task}`);
}

// ── assignTask ────────────────────────────────────────────────────────────────
function assignTask(ns, info, wantMoney, warfareRoster, gangInfo, config) {
  setTask(ns, info, getTask(info, wantMoney, warfareRoster, gangInfo, config));
}
...
export function processMembers(ns, gangInfo, members, warfareRoster, wantMoney, config) {
  ...
  for (const name of members) {
    ...
    assignTask(ns, info, wantMoney, warfareRoster, gangInfo, config);
  }
}
