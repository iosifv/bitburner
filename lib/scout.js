...
export function bitnodeReset(ns) {
  ...
  log(ns, "print", "SCOUT", "RESET", `${DATA_FILE} cleared — will re-scout on first tick`);
}
...
export async function reloadServers(ns, mode = "print") {
  ...
  if (existing && existing !== "NULL PORT DATA") {
    ...
    log(ns, mode, "SCOUT", "BACKUP", `${DATA_FILE} → ${BACKUP_FILE}`);
  }
  ...
  while (stack.length > 0) {
    ...
    if (host !== "home") {
      ...
      if (!ns.hasRootAccess(host)) {
        log(ns, mode, "SCOUT", "WARN", `${host} — could not root, skipping`);
        continue;
      }
    ...
    log(ns, mode, "SCOUT", "SCAN", `${host.padEnd(20)} RAM: ${`${serverMaxRam}GB`.padStart(8)}  $: ${ns.format.number(serverMaxMoney).padStart(10)}`);
  ...
  log(ns, mode, "SCOUT", "DONE", `${servers.length} servers written to ${DATA_FILE}`);
...
export function getServers(ns, filter = "all") {
  ...
  if (!raw || raw === "NULL PORT DATA") {
    log(ns, "tprint", "SCOUT", "ERROR", `${DATA_FILE} not found — run reloadServers() first`);
    return [];
  }
  ...
  try {
    ...
    log(ns, "tprint", "SCOUT", "ERROR", `${DATA_FILE} is corrupt — run reloadServers() first`);
    return [];
  }
...
export function printDiff(ns, mode = "print") {
  ...
  if (!anyChanges) {
    log(ns, mode, "SCOUT", "DIFF", "no changes since last scan");
    return;
  }

  if (diff.added.length)
    log(ns, mode, "SCOUT", "DIFF", `new servers:    ${diff.added.join(", ")}`);

  if (diff.removed.length)
    log(ns, mode, "SCOUT", "DIFF", `removed:        ${diff.removed.join(", ")}`);

  for (const u of diff.upgraded)
    log(ns, mode, "SCOUT", "DIFF", `upgraded:       ${u.name.padEnd(20)} ${u.old}GB → ${u.new}GB`);

  if (diff.newVictims.length)
    log(ns, mode, "SCOUT", "DIFF", `new victims:    ${diff.newVictims.join(", ")}`);

  if (diff.lostVictims.length)
    log(ns, mode, "SCOUT", "DIFF", `lost victims:   ${diff.lostVictims.join(", ")}`);
}
