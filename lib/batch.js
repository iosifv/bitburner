...
export async function dispatch(ns, mode = "print") {
  ...
  log(ns, mode, "OVERLORD", "TARGET", `${best.name.padEnd(25)} score: ${best.score.toFixed(2).padStart(10)}`);
  ...
  log(ns, mode, "OVERLORD", "TOTAL", `procs: ${String(totalProcs).padStart(4)}  threads: ${String(totalThreads).padStart(5)}`);
}
