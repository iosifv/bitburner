...
export async function tickTelepathy(ns, mode = "print") {
  const cap       = getConfig(ns, "telepathy-size");
  const homeReserve = getConfig(ns, "engine-free-home-ram");
  const quiet = mode === "port" ? "silent" : mode;

  const zombies = getServers(ns, "zombies");
  if (zombies.length === 0) {
    log(ns, mode, "TELEPATHY", "ERROR", "no zombie servers");
    return;
  }
  ...
  if (!scriptRam || scriptRam <= 0) {
    log(ns, mode, "TELEPATHY", "ERROR", `cannot read RAM cost for ${BRAINWORM_SCRIPT}`);
    return;
  }
  ...
  if (runningGB > cap) {
    ...
    log(ns, mode, "TELEPATHY", "RESET", `over cap (${runningGB.toFixed(0)}GB > ${cap}GB) — killed all, re-allocating`);
  }
  ...
  for (const z of zombies) {
    ...
    // Cap reached — kill any brainworm still hoarding RAM on remaining zombies
    if (totalGB >= cap) {
    ...
    // Budget for NEW threads only: subtract already-running brainworm on this server
    ...
    // Budget wants more threads but bacteria is claiming the space — evict to make room
    ...
    if (currentThreads > 0 && extraThreads <= 0) {
      log(ns, quiet, "TELEPATHY", "OK",
        `${server.padEnd(25)} threads: ${String(currentThreads).padStart(4)}  (full)`);
      totalProcs++;
      totalThreads += currentThreads;
      totalGB      += currentThreads * scriptRam;
      activeServers.push({ server, threads: currentThreads, gb: currentThreads * scriptRam });
      continue;
    }
    ...
    if (pid === 0) {
      log(ns, mode, "TELEPATHY", "WARN",
        `${server.padEnd(25)} exec() failed  threads: ${String(extraThreads).padStart(4)}`);
    } else {
      log(ns, quiet, "TELEPATHY", "SHARE",
        `${server.padEnd(25)} threads: ${String(extraThreads).padStart(4)}  pid: ${String(pid).padStart(5)}`);
      totalProcs++;
      totalThreads += currentThreads + extraThreads;
      totalGB      += (currentThreads + extraThreads) * scriptRam;
      activeServers.push({ server, threads: currentThreads + extraThreads, gb: (currentThreads + extraThreads) * scriptRam });
    }
  }

  log(ns, mode, "TELEPATHY", "TOTAL",
  ...
  for (const entry of activeServers) {
    log(ns, mode, "TELEPATHY", "WORM",
      `${entry.server.padEnd(25)} ${entry.gb.toFixed(1).padStart(7)}GB  threads: ${String(entry.threads).padStart(4)}`);
  }
