...
const HARD_CAP = 100_000; // game-crash guard (docs warn ~100k parallel batches)
...
export class RamAllocator {
  ...
  concurrentCap(totalFreeGB, planRamGB) {
    const byRam = planRamGB > 0 ? Math.floor(totalFreeGB / planRamGB) : 0;
    return Math.min(byRam, HARD_CAP);
  }
}
