...
export class BatchScheduler {
  ...
  async tick({ target, zombies, snapshot, config, nextBatchId, mode }) {
    const ns = this.ns;
    const S  = ns.getServer(target.name);
    const ctx = { ns, target, S, zombies, snapshot, config, nextBatchId, mode };
  ...
  async #tickPrepWeaken({ ns, target, S, zombies, snapshot, config, mode }) {
    ...
    log(ns, mode, "OVERLORD", "PREP-W",
      `${name}  deficit:${deficit.toFixed(2)}  weaken:${launched}/${threads}t (${toPlace} this tick)`);
  ...
  async #tickPrepGrow({ ns, target, S, zombies, snapshot, config, mode }) {
    const name = target.name;
    ...
    log(ns, mode, "OVERLORD", "PREP-G",
      `${name}  grow:${gLaunched}/${prep.growToMax}t (${growToPlace} this tick)  weaken:${wLaunched}/${weakenToPlace}t`);
  ...
  async #tickBatch({ ns, target, S, zombies, snapshot, config, nextBatchId, mode }) {
    ...
    for (const { batchId, placements } of launches) {
      const hLaunched  = await this.#execGroup(ns, HACK_SCRIPT,   placements.hack,    [name, plan.offsets.hack,    batchId, "H"]);
      const w1Launched = await this.#execGroup(ns, WEAKEN_SCRIPT, placements.weaken1, [name, plan.offsets.weaken1, batchId, "W1"]);
      const gLaunched  = await this.#execGroup(ns, GROW_SCRIPT,   placements.grow,    [name, plan.offsets.grow,    batchId, "G"]);
      const w2Launched = await this.#execGroup(ns, WEAKEN_SCRIPT, placements.weaken2, [name, plan.offsets.weaken2, batchId, "W2"]);
      if (hLaunched > 0 && w1Launched > 0 && gLaunched > 0 && w2Launched > 0) batchesLaunched++;
    }
    ...
    log(ns, mode, "OVERLORD", "BATCH", msg);
  ...
  async #execGroup(ns, script, placements, args) {
    ...
    for (const { host, threads } of placements) {
      ...
      if (pid > 0) {
        ...
        log(ns, "port", "OVERLORD", "WARN",
          `exec failed: ${script} on ${host} (${threads}t)`);
      }
    }
    return launched;
  }
